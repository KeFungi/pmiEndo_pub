phyloglmstep_fixed <-
function (full.formula, starting.formula, data, phy, 
          method = c("logistic_MPLE", "logistic_IG10"), trace = 2, 
          k = 2, ...) 
{
  ...
  fit <- function(plm) {
    return(
      phyloglm(create.formula(plm), data=data, phy=phy, 
               method = method, ...)
    )
  }
  ...
  while (flag == 0) {
    ...
    for (i in 1:p) {
      ...
      do.update = FALSE
      if ((plm.current[i] == 1) && (covariates[i] %in% terms.drop)) {
        plm.propose[i] = 0
        do.update = TRUE
      }
      if ((plm.current[i] == 0) && (covariates[i] %in% terms.add)) {
        plm.propose[i] = 1
        do.update = TRUE
      }
      if (do.update) {
